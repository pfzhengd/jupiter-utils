...
test('broadcaster 订阅一个频道', () => {
  const broadcaster = Broadcaster()
  const commit = (data) => {
    console.log('收到广播回来的数据commit', data)
  }
  const commit1 = (data) => {
    console.log('收到广播回来的数据commit1', data)
  }
...
test('broadcaster 订阅一个频道,多个消息', () => {
  ...
  broadcaster.subscribe('114', (data) => {
    console.log('收到广播回来的数据1', data)
  })
  broadcaster.subscribe('114', (data) => {
    console.log('收到广播回来的数据2', data)
  })
  broadcaster.publish('114', 'data')
...
test('broadcaster 订阅多个频道,多个消息', () => {
  const broadcaster = Broadcaster()
  broadcaster.subscribe('114', (data) => {
    console.log('收到广播回来的数据114-1', data)
  })
  broadcaster.subscribe('114', (data) => {
    console.log('收到广播回来的数据114-2', data)
  })
  broadcaster.publish('114', 'data')
  broadcaster.unsubscribe('114')
